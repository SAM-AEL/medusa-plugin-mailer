...
- **Admin UI Mapping**: Map Medusa events (like `order.placed`) to specific `.html` templates directly from the admin dashboard.
...
- **Workflow-Based Delivery**: Event subscribers trigger a Medusa workflow, so email delivery can run on your dedicated worker instance instead of blocking API requests.
- **Manual Sending**: A test utility in the admin UI to send one-off HTML template emails based on dynamic data.
- **No DB Clutter**: Connects to SMTP via secure environment variables. Does not create bulky log tables in your database (relies on Medusa’s core logger).
...
This plugin is built for **Medusa v2.12.x**.
...
2. Add the plugin to your `medusa-config.ts` plugins array:
```typescript
...
3. Run migrations to create the required mapping table:
```bash
...
The plugin natively intercepts the following core Medusa events for entity data enrichment:
- `order.placed`, `order.completed`, `order.canceled`, `order.updated`, `order.fulfillment_created`
- `fulfillment.created`, `fulfillment.shipment_created`, `fulfillment.delivery_created`
